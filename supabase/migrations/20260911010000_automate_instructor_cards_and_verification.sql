-- AUTOMATED INSTRUCTOR SYSTEM & VERIFICATION RULE (MASTRIVE)
-- 1. Applications immediately create published instructor cards without verified badge
-- 2. Verified badge is only unlocked when learners_count >= 10
-- 3. Row Level Security (RLS) is fully configured for anonymous and authenticated access

-- ==========================================
-- 1. Table schema updates
-- ==========================================

-- Ensure instructor_applications has all required fields
...
-- Ensure instructors table exists and has proper schema
...
-- In case instructors table was already created with strict NOT NULL on user_id, make it nullable
...
-- ==========================================
-- 2. Automatic Card Creation & Sync Trigger
-- ==========================================
...
as $$
declare
  calc_verified boolean;
  calc_learners integer;
begin
  calc_learners := coalesce(new.learners_count, 0);
  calc_verified := (calc_learners >= 10);
  ...
  insert into public.instructors (
    ...
    0, -- starts with 0 learners
    false, -- verified is false until 10 learners boarded
    true, -- automatically published so instructor card is created
    now(),
    now()
  )
...
-- Trigger to maintain 10-learner verification rule on instructors table
...
-- instructor_applications policies
...
-- ==========================================
-- 4. Storage Bucket Setup & Policies
-- ==========================================
